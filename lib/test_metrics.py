...
def plot_acf(acf_real,acf_fake):
    dim=acf_real.shape[-1]
    fig, ax = plt.subplots(dim, 1)
    if dim==1:
      ax.plot(acf_real[:],label='real')
      ax.plot(acf_fake[:],label='fake')
      ax.legend()
    else:
      for i in range(dim):
        ax[i].plot(acf_real[:,i],label='real')
        ax[i].plot(acf_fake[:,i],label='fake')
        ax[i].legend()
...
class ACFLoss(Loss):
    ...
    def get_acf(self, x_fake):
        acf_fake = acf_torch(self.transform(x_fake), self.max_lag)
        return acf_fake 
        
    def plot(self,x_fake):
        acf_fake = acf_torch(self.transform(x_fake), self.max_lag)
        plot_acf(self.acf_real.cpu(),acf_fake.cpu())


class ACFLoss_dim(Loss):
    def __init__(self, x_real, max_lag=64, **kwargs):
        super(ACFLoss_dim, self).__init__(norm_foo=acf_diff, **kwargs)
        self.x_real=x_real
        self.acf_real = acf_torch(self.transform(x_real), max_lag, dim=(0, 1))
        self.max_lag = max_lag
    ...
    def forward(self, x_fake):
        self.loss_componentwise = self.compute(x_fake)
        return self.reg * self.loss_componentwise
...
test_metrics = {
    ...
    'acf_dim': partial(ACFLoss_dim, name='acf_dim'),
