...
def get_arch_dataset(window_size, lag=4, bt=0.055, N=5000, dim=1):
    ...
    def get_raw_data(N=5000, lag=4, T=2000, omega=0.00001, bt=0.055, burn_in=2000):
        ...
        return  np.concatenate([logrtn[:, burn_in:,None],arch[:, burn_in:,None]], axis=-1)
                                                  
    pipeline = Pipeline(steps=[('standard_scale', StandardScalerTS(axis=(0, 1)))])
    raw = get_raw_data(T=window_size, N=N, bt=bt)
    data_raw = torch.from_numpy(raw).float()
...
def rolling_back(x, x_lag):
    #input has shape (B,x_lag,C)
    T=x.shape[0]
    return torch.cat([x[i:i+1, : ] for i in range(T)], dim=1)
...
def rolling_back(x, x_lag):
    ...
    for i in range(T):
        if i%x_lag==0:
          x_long.append(x[i:i+1, : x_lag])
          m=i
    if m+1<T:
        x_long.append(x[-1:, -(T-m): ])
    return torch.cat(x_long, dim=1)
...
def get_data(data_type, p, q,pip=False, **data_params):
    if data_type == 'VAR':
...
def parse_parameter(params,dim,p,q):
    k=1
    v=k+p+1
    m=v+q
    mean,alphas,betas=params[:k], params[k :v ], params[v :m]
